...
export const meta: MetaFunction = () => createPostMeta('compliance-as-code');
  ...
  ko: {
    ...
    body: (
      <>
        ...
        <p>1세대 Harness 규칙들은 딱 예상 가능한 것들을 검사했다: domain 폴더가 있는가, Interface 계층이 Infrastructure를 직접 import하지 않는가, Repository 인터페이스가 문서에 명시된 위치에 있는가. 이것만으로도 한 부류의 드리프트 전체를 잡아낼 수 있다 — 그리고 정확히 그만큼의 사각지대도 존재한다. 올바른 파일 안에서 메서드 이름이 올바르게 지어졌는지, 올바른 폴더 안의 클래스가 올바른 것에 의존하는지에 대해서는 아무것도 말해주지 않는다.</p>
        <p>실제 감사에서 정확히 이 틈이 드러났다: Repository 메서드 네이밍 컨벤션(목록 조회는 항상 <code>find&lt;Noun&gt;s</code>, 저장은 항상 <code>save&lt;Noun&gt;</code>)이 다섯 언어 구현 중 네 곳에서, 매번 다른 방식으로 위반되어 있었다 — 그중 두 곳은 명사 없이 <code>save</code>만 사용하고 있었고, 네 곳은 더 오래된 도메인에 "전용 <code>findOne</code>과 별도의 <code>findAll</code>" 쌍이 되살아나 있었다. 그 파일들은 하나같이 당시 존재하던 모든 구조적 규칙을 통과했는데, 그 규칙들 중 어느 것도 메서드 이름을 들여다본 적이 없었기 때문이다.</p>
        ...
        <p>더 미묘한 사각지대도 있다: 언어를 하나씩 순서대로 감사하는 방식으로는 언어 <em>사이에만</em> 존재하는 구조적 불일치를 절대 볼 수 없다. 알림 발송이라는 관심사가 다섯 언어 구현 중 두 곳에서는 domain 모듈 안에, 나머지 세 곳에서는 별도의 공유 최상위 모듈에 자리 잡고 있었다 — 어느 쪽 선택도 그 자체만 보면 명백히 틀린 건 아니었고, 각자 자기 언어의 Harness는 깔끔하게 통과했으며, 이 불일치는 애초에 단일 언어 리뷰만으로는 구조적으로 드러날 수 없었다. 같은 개념을 다섯 언어 전체에 걸쳐 나란히 비교했을 때에야 비로소 드러났는데, 이런 비교는 "이 코드베이스 하나를 자신의 문서와 비교해 리뷰한다"는 것과는 근본적으로 다른 종류의 감사다.</p>
        <h2>발견 사항을 영구적인 규칙으로 바꾸기</h2>
        <p>여러 라운드에 걸쳐 유지된 패턴은 이랬다: 실제 위반 사례를 손으로 한 번 찾아내고, 고치고, 그것을 잡아냈을 Harness 규칙을 작성한 뒤 — 코드베이스가 이제 깨끗하다고 가정하기 전에 그 새 규칙을 즉시 실행하는 것. Repository 네이밍 규칙이 가장 명확한 사례다: 그것이 프로즈가 아니라 기계적 검사로 존재하게 되자마자, 아무도 다시 확인할 생각을 못 했던 도메인 — 다섯 언어 중 세 곳의 인증(authentication) 도메인 — 에 돌려보니 이전의 어떤 감사 범위에도 포함된 적 없던 실제 위반 사례 세 건이 곧바로 드러났다. 모든 이전 감사가 사람들이 계속 신경 쓰던 두 비즈니스 도메인으로만 범위가 한정돼 있었기 때문이다. 의존성 방향 규칙, ID 포맷 규칙, 에러 응답 스키마 규칙, soft-delete 필터 규칙 — 다섯 언어에 걸쳐 이런 식으로 약 서른 개의 규칙이 쌓였고, 각각은 이 사례처럼 가상의 버그가 아니라 실제 버그에서 태어났다.</p>
        <p>이 수확은 시간이 지날수록 줄어드는데, 이는 예상된 일이지 이 작업이 더 이상 가치가 없어졌다는 신호가 아니다. 초기 라운드에서는 새 규칙 카테고리마다 실제 위반 사례를 서너 건씩 찾아냈지만, 이 작업의 네 번째 라운드에 이르러서는 새 규칙 대부분이 아무것도 찾아내지 못했다. 손쉽게 잡을 수 있는 언어 간 드리프트가 이미 정리되었기 때문이다. 수확 체감이지, 수확이 0이 되는 건 아니다 — 하루 오후를 들여 작성했지만 오늘은 아무것도 잡아내지 못하는 규칙이라도, 다음 달 찾아올 회귀를 막아내는 파수꾼 역할은 여전히 하고 있는 셈이다.</p>
