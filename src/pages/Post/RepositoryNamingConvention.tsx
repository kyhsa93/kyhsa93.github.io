...
export const meta: MetaFunction = () => createPostMeta('repository-naming-convention');
...
if (!order) throw new Error(OrderErrorMessage['Order not found.'])`}</code></pre>
        ...
        <p>이 특정 컨벤션을 위한 자동화된 검사는 존재하지 않았다. 이 저장소의 Harness들은 구조적 배치를 검사한다 — Repository 인터페이스가 <code>domain/</code>에 있는지, 구현체가 <code>infrastructure/</code>에 있는지, Interface 계층이 Infrastructure를 직접 건드리지 않는지 — 하지만 메서드 이름이 <code>save</code>인지 <code>saveOrder</code>인지는 그 어느 것도 말해주지 않는다. 글로만 남아 있는 네이밍 컨벤션은 사람들이 그 글을 다시 읽어야겠다고 기억하는 동안만 지켜진다 — 즉 현실에서는 두 번째 사람이 그 파일을 건드리기 전까지, 혹은 다른 문단을 먼저 읽은 누군가가 네 번째 언어 포트를 작성하기 전까지만 유효하다는 뜻이다.</p>
