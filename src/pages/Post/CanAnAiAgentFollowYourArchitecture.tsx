  ...
  ko: {
    ...
    body: (
      <>
        ...
        <p>에이전트에게 주어지는 프롬프트에는 정확히 세 가지만 담겨 있고, 구현 방법에 대해서는 아무것도 알려주지 않는다: 아직 저장소에 존재하지 않는 도메인의 비즈니스 규칙; 저장소의 기존 컨벤션을 따르라는 지시와 어디서부터 읽어야 하는지에 대한 최소한의 진입점 — 다른 문서 경로는 전혀 주지 않고 해당 언어의 <code>CLAUDE.md</code>만 제공한다; 그리고 완료 기준, 이는 그저 Harness 자체를 실행해 통과할 때까지 반복하라는 것뿐이다. 읽을 목록을 건네받는 대신 문서 인덱스를 따라가면서 에이전트가 스스로 관련 문서를 찾아내는지 여부 자체가 측정 대상의 일부다.</p>
        ...
        <p>첫 실전 실행에서는 에이전트에게 Subscription 도메인을 줬다 — owner와 plan 이름, 생성 시 <code>PENDING</code> 상태, 단순한 <code>activate()</code> 전이, 그리고 "다른 부분이 여기에 반응해야 할 수도 있다"고만 의도적으로 설명해 놓고 기술적으로 그게 무슨 뜻인지는 더 이상 힌트를 주지 않은 <code>cancel(reason)</code>. scaffolding generator나 reference template을 가리키는 언급은 어디에도 없었다.</p>
        ...
        <p>모든 언어가 완벽한 점수를 받았고 — 완벽한 점수 자체보다 더 흥미로운 지점은 — 모든 구현체가 독립적으로 Domain Event를 오직 <code>expire()</code>에만 붙이고 <code>redeem()</code>에는 붙이지 않기로 선택했다는 것이다. 이는 루트 문서 하나가 단 한 번 설명해 놓은 것과 동일한 근본 패턴("아무도 반응하지 않는 전이에는 이벤트가 없고, 무언가가 반응해야 하는 전이에는 이벤트가 있다")을 그대로 적용한 것이다. 문서 하나, 독립된 에이전트 다섯, 동일한 아키텍처 판단 하나.</p>
        <p>이번 실행에서 더 중요한 결과는 Java의 불일치다. 독립 재검증이 자체 보고와 어긋났는데, 원인은 코드가 아니었다 — 오래된 Gradle 빌드 캐시 디렉터리가 Harness에 의해 마치 실제 소스인 것처럼 스캔되면서 거짓 "레이어 디렉터리 없음" positive를 만들어냈다. 빌드 산출물을 삭제하고 다시 실행하자 자체 보고와 정확히 일치했다. 이는 "자체 보고를 절대 그대로 믿지 말라"는 규칙이 존재하는 정확한 이유를 보여주는 사례다: 불일치는 실제 버그를 가리키고 있었지만, 아무도 예상하지 못한 버그였을 뿐이다.</p>
