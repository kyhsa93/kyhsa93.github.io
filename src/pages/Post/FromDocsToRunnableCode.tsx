...
export const meta: MetaFunction = () => createPostMeta('from-docs-to-runnable-code');
...
# → A (100/100)`}</code></pre>
  ...
  ko: {
    ...
    body: (
      <>
        <p>이 저장소의 <code>docs/reference.md</code>는 실용적인 구현 템플릿을 정의한다 — 모든 계층, 모든 파일, 모든 네이밍 컨벤션을 한곳에서 보여주는 작은 예시(과거에는 Order 도메인이었다)다. 문서로 작성된 템플릿은 유용하다 — 하지만 다섯 번째로 연이어 새 도메인을 만들 때도 그 내용을 사람이 일일이 손으로 정확하게 타이핑해야 한다면, 거기서부터는 한계가 드러난다. 다음 단계는 그 템플릿을 생성기(generator)로 바꾸는 것이었다: 도메인 이름 하나만 받아서 실제로 Harness를 통과하는 코드를 만들어내는 스크립트다.</p>
        <h2>한 번에 생성되는 것들</h2>
...
# → A (100/100)`}</code></pre>
        ...
        <p>이런 일은 실제로 한 번이 아니라 여러 번 벌어졌다. Repository 메서드 네이밍 규칙이 도입되었을 때(흩어져 있던 패턴들을 <code>find&lt;Noun&gt;s</code>/<code>save&lt;Noun&gt;</code>/<code>delete&lt;Noun&gt;</code>로 통일) — 전용 감사가 아니라 무관한 5개 언어 벤치마크 실행 도중 — Go와 kotlin-springboot 생성기 둘 다 여전히 예전의 find-by/명사 없는 단순 save 형태를 만들어내고 있다는 사실이 드러났다. 그 규칙이 배포된 이후로 둘 다 손댄 적이 없었기 때문이다. 둘 다 실제 도메인들이 이미 쓰고 있던 <code>Find&lt;Domain&gt;/FindOne</code> + <code>Save&lt;Domain&gt;</code> 컨벤션으로 다시 작성됐다. NestJS의 request-scope 기반 user-context 저장소(<code>@Authenticated()</code> + <code>UserContextStore.getRequesterId()</code>)가 <code>req.user</code> 직접 접근 방식을 대체했을 때도, NestJS 생성기의 Controller 템플릿은 예전 <code>req.user</code> 패턴을 계속 만들어냈고 새 도메인을 스캐폴딩하는 순간 Harness에서 떨어졌다 — 손으로 작성한 예시에 적용된 수정과는 별개의 수정이었다. 둘은 같은 산출물이 아니기 때문이다. Outbox 패턴이 다섯 언어 전체에서 단일 패스 드레인에서 다중 패스 드레인으로 바뀌었을 때도 똑같은 일이 반복됐다: 모든 생성기가 손으로 작성한 예시와 동일한 구조적 수정을, 별도의 두 번째 커밋으로 필요로 했다.</p>
        ...
        <p>이 저장소의 모든 언어는 결국 각자의 생태계에 맞는 관용적인 방식으로 이 도구의 자체 버전을 갖게 됐다 — NestJS는 Node 스크립트, Go는 <code>go run .</code>을 쓰는 독립된 Go 모듈(Go에는 기존 모듈에 스캐폴딩 스크립트를 자연스럽게 붙일 자리가 없기 때문이다), 두 개의 Spring Boot 포트는 Python 스크립트(파일 하나 스캐폴딩하자고 Java/Gradle 툴체인을 부팅시키게 하는 대신 의도적으로 Python을 선택했다), 그리고 FastAPI용도 하나. 다섯 개 모두 동일한 계약을 따른다 — 도메인 이름을 입력받고, 붙여넣을 스니펫만 출력하는 대신 새 도메인을 자동으로 등록하는 선택적 <code>--wire</code> 플래그와, 완전히 다른 프로젝트를 대상으로 하는 <code>--out</code> 플래그를 함께 지원한다 — 이는 이 저장소를 손으로 베껴 쓸 참조 자료로서만이 아니라, 완전히 새로운 서비스를 부트스트랩하는 템플릿으로 다루는 데도 쓸모가 있다.</p>
