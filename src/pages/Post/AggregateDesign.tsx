...
export const meta: MetaFunction = () => createPostMeta('aggregate-design');
  ...
  ko: {
    ...
    body: (
      <>
        ...
        <p>Aggregate Root는 비즈니스 규칙과 불변식을 캡슐화한다. 외부의 그 무엇도 그 내부 상태를 직접 바꾸지 못한다 — 변경은 항상 자신의 도메인 메서드를 거쳐야 하고, 불변식이 깨지면 나중에 어딘가 다른 곳이 아니라 바로 그 메서드 안에서 즉시 예외가 발생해야 한다.</p>
        <pre><code>{`// domain/OrderException.kt
sealed class OrderException(message: String) : RuntimeException(message)
class OrderMustHaveItemsException : OrderException("An order must have at least one item.")
class OrderAlreadyCancelledException : OrderException("This order has already been cancelled.")
...
class Order private constructor() {
    ...
    fun cancel(reason: String) {
        ...
        <p>Application Service는 비즈니스 로직을 스스로 수행하지 않는다 — Aggregate의 메서드에 위임할 뿐, 그 이상은 아니다. Command Service 안에서 도메인에 관한 <code>if</code> 문을 쓰고 있다면, 그 로직은 거의 확실히 한 계층 아래로 내려가야 한다.</p>
        <h2>다른 Aggregate는 객체가 아니라 반드시 ID로 참조하라</h2>
        <p>트랜잭션 경계는 Aggregate Root 단위로 설정된다 — 트랜잭션 하나당 오직 하나의 Aggregate만 변경된다. 이는 Aggregate들이 서로를 직접 객체 참조로 붙들고 있지 않을 때만 가능하다. <code>Order</code>는 <code>userId: String</code>을 가질 뿐, <code>User</code> 객체를 갖지 않는다. 객체를 직접 참조하면 ID로 참조할 때는 생기지 않을 결합이 생긴다: Aggregate 하나를 로드할 때 단지 타입을 맞추기 위해 다른 객체들의 그래프까지 줄줄이 로드되는 일이 없어야 한다.</p>
...
data class Money(val amount: Long, val currency: Currency) {
    ...
    fun add(other: Money): Money {
        ...
        <p>경계가 정말로 애매하다면 작게 시작하라. 두 Aggregate가 프로덕션에서 실제로 어떻게 변화하는지 지켜본 뒤 나중에 합치는 편이, 이미 비대해진 것을 부하가 걸린 상태에서 억지로 쪼개려 애쓰는 것보다 비용이 훨씬 적게 든다.</p>
...
class Order private constructor() {
    ...
    companion object {
        ...
        <p>이 모든 것은 유일하게 올바른 다이어그램 하나를 찾는 일이 아니다. 규칙을 지키는 단위를 그 규칙이 요구하는 만큼만 정확히 유지하는 일이다 — 관련 없는 데이터를 매번 락에 끌어들이지 않도록 너무 크지 않게, 그리고 그 규칙이 지켜야 할 대상이 우연히 처음 호출한 Service로나 새어나가지 않도록 너무 작지도 않게.</p>
