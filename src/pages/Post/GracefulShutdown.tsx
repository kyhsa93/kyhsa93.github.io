...
✗ Release the DB connection → shut down the HTTP server   (in-flight requests can't use the DB)`}</code></pre>
  ...
  ko: {
    ...
    body: (
      <>
        <p>컨테이너 오케스트레이션 환경 — Kubernetes, ECS, 대신 컨테이너를 시작하고 종료해 주는 무엇이든 — 에서는 모든 배포, 모든 오토스케일 다운, 모든 노드 드레인이 동일한 신호를 보낸다: SIGTERM, 그리고 그 뒤에 SIGKILL이 뒤따르기까지의 카운트다운. 그 시간 동안 프로세스가 무엇을 하느냐가 아무도 눈치채지 못하는 배포와, 어딘가의 대시보드에 실패한 요청이 급증하는 배포의 차이를 만든다.</p>
        ...
        <p>readiness 전환은 HTTP 서버가 종료되기 전에 일어나야 한다. 순서를 뒤바꾸면, 로드밸런서가 여전히 해당 인스턴스를 정상으로 여기고 이미 활발히 종료 중인 서버로 트래픽을 계속 라우팅하는 구간이 생긴다 — 이는 정확히 배포 중에만 간헐적으로 나타나고 코드 변경과는 무관해 보이는 "배포 중 랜덤 502" 장애의 전형적인 모양새다.</p>
...
return 200  // always`}</code></pre>
        ...
        <h2>terminationGracePeriodSeconds</h2>
        <p>이는 오케스트레이터가 SIGTERM 이후 SIGKILL로 escalation하기까지 기다리는 시간이며, SIGKILL은 그 이상의 정리 작업 없이 프로세스를 강제 종료한다. 서비스의 p99 요청 처리 시간보다 여유 있게 잡아라 — 일반적인 HTTP 서비스라면 보통 30초면 충분하다. 종료 시점에 배치나 스케줄 작업이 진행 중일 수 있다면, HTTP p99뿐 아니라 그 작업들의 최대 처리 시간도 함께 감안해야 한다.</p>
        <pre><code>{`# Kubernetes example
spec:
  terminationGracePeriodSeconds: 30
  containers:
...
// DB connections are cleaned up only after the HTTP server is fully closed`}</code></pre>
        <p>여기 있는 것 중 Go만의 특별한 노하우는 없다 — 이 글 맨 위에서 소개한 여섯 단계와 똑같고, 다만 순서를 감춰 주는 프레임워크가 없을 뿐이다. SIGTERM 시 <code>ctx</code>가 취소되는 것은 프로세스 안의 다른 모든 백그라운드 루프 — Outbox poller, Task Queue consumer, 스케줄러 — 를 멈추는 계기이기도 하다. 모두가 같은 context를 지켜보고 있기 때문에, 신호 하나로 컴포넌트마다 별도의 shutdown hook 없이도 모든 것이 깔끔하게 정리된다.</p>
