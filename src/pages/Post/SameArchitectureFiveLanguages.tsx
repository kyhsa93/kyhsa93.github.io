...
class AccountRepository(AccountQuery, ABC):
    ...
    fun hasTransactionWithReference(referenceId: String, type: TransactionType): Boolean
        ...
        <div className="article-note"><strong>진짜 핵심</strong><p>"프레임워크에 독립적인 아키텍처"라는 게 코드가 어디서나 똑같이 생겼다는 뜻은 아니다 — 명백히 그렇지 않다. 그것은 각 생태계 고유의 관용구 안에서 같은 결정이 독립적으로 내려진다는 뜻이다: 어떤 메서드가 쓸 수 있는지, 어떤 메서드는 읽기만 할 수 있는지, 그리고 그 경계가 관례만이 아니라 타입 시스템에 의해 강제되는 지점이 어디인지. 위의 다섯 구현은 정확히 같은 질문 — "내 언어로는 이걸 어떻게 표현하는가" — 에 대한 다섯 가지 다른 답이다.</p></div>
