...
'2026-08-05T00:00:00.000+00:00'   // same process, TZ=UTC`}</code></pre>
        ...
        <p>Five languages now compute one thing the same way for the first time in the repository's history — not because they share a clock library, or because "just use UTC" was ever in doubt as the right rule. Because each one finally has the specific shape of fix its own runtime actually needed: a corrected reading at the call site in four of them, and a corrected process in the fifth, for a reason that was worth learning once rather than papering over with the same fix copied five times.</p>
...
'2026-08-05T00:00:00.000+00:00'   // same process, TZ=UTC`}</code></pre>
        ...
        <p>5개 언어는 이제 이 저장소 역사상 처음으로 같은 걸 같은 방식으로 계산한다 — 같은 시계 라이브러리를 공유해서가 아니고, "그냥 UTC 쓰면 되잖아"가 애초에 옳은 규칙인지 의심스러웠던 적도 없다. 각자가 마침내 자기 런타임이 실제로 필요로 했던 정확한 모양의 수정을 갖게 됐기 때문이다: 넷에서는 호출 지점에서의 교정된 읽기, 다섯 번째에서는 교정된 프로세스 — 같은 수정을 다섯 번 복사해 덮는 대신, 한 번은 제대로 이해할 가치가 있는 이유로.</p>
