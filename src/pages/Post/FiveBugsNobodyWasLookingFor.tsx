...
const content = {
  en: {
    ...
    body: (
      <>
        ...
        <div className="article-note"><strong>The pattern underneath all seven</strong><p>None of these bugs were about documentation. Every one of them was already sitting in production-shaped code, waiting for a request shaped exactly the way its author never happened to send one. What found all seven wasn't a smarter reviewer — it was a rule applied uniformly: don't just make the annotation compile, prove the thing it describes is actually true by asking the running app.</p></div>
  ...
  ko: {
    ...
    lede: 'nestjs의 미완성 Swagger 문서를 고치는 데는 반나절이면 충분했다. 애노테이션이 컴파일된다는 걸 믿는 대신, 실제로 앱을 켜고 문서화된 모든 에러 경로를 curl로 확인해서 수정을 검증했더니 — 문서와는 아무 관계 없는 버그가 하나 나왔다. 같은 원칙을 나머지 4개 언어에 그대로 적용하자 네 개가 더 나왔고, 그중 하나는 실제 배포라면 스키마 마이그레이션이 조용히 멈춰 있었을 만큼 심각했다. 그 개수와는 별개로 과정에서 두 개의 버그가 더 얻어걸렸다 — 못지않게 진짜였다.',
    body: (
      <>
        ...
        <p>나머지는 더 작지만 전부 진짜였고, 개수를 채웠다: kotlin의 Spring Security는 인증되지 않은 요청에 자체 일반 403을 반환해버려서, 앱 자신의 예외 핸들러가 문서화된 401 형태를 만들 기회조차 얻지 못했다. go의 인증 미들웨어는 401을 자기 문서가 약속한 JSON 스키마가 아니라 평문으로 보냈고, 여러 validation 400도 같은 문제였다. fastapi는 잘못된 JWT에 대한 예외 핸들러 자체가 없어서 — 잘못된 토큰이 깔끔한 401 대신 처리되지 않은 500을 만들어냈다. 여기까지가 제목이 세는 다섯 개다. 그 개수 밖에서, 못지않게 진짜인 두 개가 과정에서 더 나왔다: java 자신의 <code>/v3/api-docs</code>와 Swagger UI는 보려면 bearer 토큰이 필요했다 — API 문서 자체가 공개적으로 접근 불가능했던 것이다. 그리고 nestjs 자신의 스캐폴딩 생성기는, 이번 Swagger 수정이 건드리고 있던 바로 그 코드 안에 버그가 있었다: 생성된 cancel 핸들러의 "이미 취소됨" 도메인 에러가 컨트롤러의 catch 블록에서 한 번도 매핑된 적이 없어서, 의도한 400 대신 조용히 500을 만들어내고 있었다.</p>
        <div className="article-note"><strong>일곱 개 모두를 관통하는 패턴</strong><p>이 버그 중 어느 것도 문서화에 관한 게 아니었다. 전부 이미 프로덕션 형태의 코드 안에 앉아, 저자가 한 번도 우연히 보내본 적 없는 정확한 모양의 요청을 기다리고 있었다. 이 일곱 개를 찾아낸 건 더 똑똑한 리뷰어가 아니었다 — 균일하게 적용된 규칙 하나였다: 애노테이션을 컴파일되게 만드는 것으로 끝내지 말고, 그것이 서술하는 게 실제로 참인지 돌아가는 앱에 직접 물어봐서 증명하라.</p></div>
