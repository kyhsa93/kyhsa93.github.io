...
export const meta: MetaFunction = () => createPostMeta('llm-technical-service');
...
const defaultOllamaBaseURL = "http://localhost:11434"`}</code></pre>
        ...
        <div className="article-note"><strong>Further reading in the repo</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/domain-service.md" target="_blank" rel="noreferrer">docs/architecture/domain-service.md</a> — the Technical Service / Domain Service split (now uses a different worked example, see the update note above) · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/22107f887cacc7735d6709e21d1967d024ee790d/implementations/go/examples/internal/infrastructure/llm/refund_reason_classifier.go" target="_blank" rel="noreferrer">refund_reason_classifier.go</a> — the Ollama-backed implementation as it existed, pinned to the last commit before removal
        </p></div>
      </>
    ),
  },
  ko: {
    ...
    body: (
      <>
        <div className="article-note"><strong>정정 — 2026.07.26</strong><p>여기서 다루는 <code>RefundReasonClassifier</code>는 이후 제거되었다. 분류 대상이었던 자유 텍스트 <code>reason</code>은 환불을 요청하는 바로 그 사람이 온전히 통제하는 값이었다 — 사기꾼을 그대로 신뢰하는 사기 탐지 신호였던 셈이다. 전체 경위와 거기서 남은 원칙은 <a href="/posts/the-fraud-signal-that-trusted-the-fraudster">사기꾼을 그대로 믿은 사기 탐지 신호</a>에 정리했다. 이 글 하단의 소스 링크는 이제 이 파일이 마지막으로 존재했던 커밋을 가리킨다.</p></div>
...
const defaultOllamaBaseURL = "http://localhost:11434"`}</code></pre>
        ...
        <div className="article-note"><strong>저장소 내 추가 자료</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/domain-service.md" target="_blank" rel="noreferrer">docs/architecture/domain-service.md</a> — Technical Service / Domain Service 구분(지금은 다른 실사례를 쓰고 있다, 위 정정 참고) · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/22107f887cacc7735d6709e21d1967d024ee790d/implementations/go/examples/internal/infrastructure/llm/refund_reason_classifier.go" target="_blank" rel="noreferrer">refund_reason_classifier.go</a> — 제거 직전 마지막 커밋에 고정된, 존재했던 Ollama 기반 구현
        </p></div>
      </>
    ),
  },
