...
export const meta: MetaFunction = () => createPostMeta('domain-services-across-aggregates');
...
class RequestRefundHandler:
    ...
    async def execute(self, cmd: RequestRefundCommand) -> Refund:
        ...
        <div className="article-note"><strong>Further reading in the repo</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/domain-service.md" target="_blank" rel="noreferrer">docs/architecture/domain-service.md</a> — the full Domain Service / Technical Service pattern, with the misuse example above · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/implementations/fastapi/examples/src/payment/domain/refund_eligibility_service.py" target="_blank" rel="noreferrer">payment/domain/refund_eligibility_service.py</a> — the real, current code (now simplified to just the two structural checks, see the update note above)
        </p></div>
      </>
    ),
  },
  ko: {
    ...
    body: (
      <>
        <div className="article-note"><strong>정정 — 2026.07.26</strong><p>아래에 나오는 <code>classification</code>과 <code>ml_fraud_risk_score</code>는 이후 <code>RefundEligibilityService</code>의 실제 시그니처에서 모두 제거되었다 — 이제 이 메서드는 <code>(payment, refund)</code>만 받는다. 두 개의 상위 Technical Service가 모두 제거되었고(<a href="/posts/the-fraud-signal-that-trusted-the-fraudster">사기꾼을 그대로 믿은 사기 탐지 신호</a> 참고), 이 Domain Service에는 이제 어떤 형태의 사기 판단도 남아있지 않으며 아래의 구조적 검사 두 가지만 남았다. 이 글이 말하는 핵심 — 어느 한쪽도 흡수할 수 없는 두 Aggregate를 조율하는 Domain Service — 은 여전히 유효하다. 낡은 건 사기 신호 파라미터 쪽이다.</p></div>
...
class RequestRefundHandler:
    ...
    async def execute(self, cmd: RequestRefundCommand) -> Refund:
        ...
        <div className="article-note"><strong>저장소 내 추가 자료</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/domain-service.md" target="_blank" rel="noreferrer">docs/architecture/domain-service.md</a> — Domain Service / Technical Service 패턴 전체와 위의 오용 사례 · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/implementations/fastapi/examples/src/payment/domain/refund_eligibility_service.py" target="_blank" rel="noreferrer">payment/domain/refund_eligibility_service.py</a> — 실제 현재 코드(이제 구조적 검사 두 가지로 단순화됐다, 위 정정 참고)
        </p></div>
      </>
    ),
  },
