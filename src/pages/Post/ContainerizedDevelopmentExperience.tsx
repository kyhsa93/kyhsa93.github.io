...
export const meta: MetaFunction = ({ location }) =>
  createPostMeta('containerized-development-experience', localeFromPathname(location.pathname));
...
volumes:
  ...
  ko: {
    ...
    body: (
      <>
        <p>“내 컴퓨터에서는 잘 되는데요”는 대개 코드가 아니라 환경의 계약(contract)이 한 번도 문서화되지 않았다는 신호다. 런타임 버전, 데이터베이스 설정, 의존 서비스, 시드 데이터, 기동 순서가 누군가의 기억 속에만 존재한다면, 팀의 개발 속도는 그 환경에 가장 익숙하지 않은 사람의 속도에 발목을 잡히게 된다.</p>
        ...
        <p>여기서 “필요한”이라는 단어가 중요하다. 프로덕션 인프라의 모든 요소를 로컬에서 그대로 재현할 필요는 없다. 계약을 검증하는 데 꼭 필요한 것부터 우선 포함시켜야 한다 — 예를 들어 개발 중인 기능이 실제로 쓰는 데이터베이스, 메시지 브로커, 캐시 같은 것들이다. Observability 플랫폼이나 대규모 분석 시스템 같은 것들은 목(mock)이나 공유 개발 환경으로 대체할 수 있다.</p>
