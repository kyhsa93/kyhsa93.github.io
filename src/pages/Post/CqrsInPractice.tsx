  ...
  ko: {
    ...
    body: (
      <>
        ...
        <h2>두 단계의 CQRS</h2>
        <p>Application Service를 Command Service와 Query Service로 나누는 것만으로도 이미 가벼운 형태의 CQRS이며, 대부분의 도메인에는 이걸로 충분하다. Handler 기반 CQRS — Command Bus / Query Bus를 도입하고 각 유스케이스를 독립된 Handler 클래스로 분리하는 방식 — 은 Service 클래스가 너무 많은 유스케이스로 비대해지거나, 쓰기 모델과 읽기 모델이 정말로 별도의 저장소여야 할 때 도입할 가치가 있다. 유스케이스가 적고 Service 클래스가 계속 단순하게 유지된다면 가벼운 형태로 충분하다 — 패턴에 이름이 붙어 있다는 이유만으로 Handler를 끌어다 쓰지 말 것.</p>
        <pre><code>{`src/
  <domain>/
    domain/                              # unchanged
    application/
    ...
    interface/
      order-controller.ts                # calls the CommandBus / QueryBus`}</code></pre>
        ...
        <p>QueryHandler는 읽기 전용 인터페이스를 사용한다 — <code>OrderRepository</code>가 아니라 <code>OrderQuery</code>다. DB를 직접 조회하며, Aggregate 재구성은 일어나지 않는다.</p>
        <pre><code>{`// application/query/order-query.ts — the Query interface (abstract class)
export abstract class OrderQuery {
  abstract getOrders(query: GetOrdersQuery): Promise<GetOrdersResult>
  abstract getOrder(query: GetOrderQuery): Promise<GetOrderResult>
...
export class GetOrdersQueryHandler {
  ...
  public async execute(query: GetOrdersQuery): Promise<GetOrdersResult> {
    return this.orderQuery.getOrders(query)
        ...
        <p>이건 그저 네이밍의 뉘앙스 차이처럼 보인다 — <code>OrderRepository</code> 대신 <code>OrderQuery</code> — 그리고 바로 그 점이 아무도 눈치채지 못한 채 규칙을 어기기 쉽게 만든다. Repository는 이미 Command 쪽에 주입되어 있고, 이미 테스트도 되어 있으며, 목록 화면이 필요로 하는 것을 정확히 반환하는 <code>findOrders</code> 메서드도 이미 갖고 있다. Query Handler에서 그걸 가져다 쓰면 컴파일도 되고 리뷰도 통과하지만, CQRS가 존재하는 이유인 바로 그 문을 조용히 다시 열어버린다: 이제 읽기 경로 바로 옆에 쓰기 능력이 놓이게 되고, 두 모델은 더 이상 실질적으로 분리되어 있지 않다.</p>
        ...
        <p>Controller 쪽에서 보면 CQRS를 도입하는 건 대부분 라우팅 변경에 불과하다 — 올바른 Service 메서드 대신 올바른 Bus를 호출할 뿐이다:</p>
        <pre><code>{`public async cancelOrder(param: CancelOrderRequestParam): Promise<void> {
  return commandBus.execute(new CancelOrderCommand(param))
    .catch((error) => { throw convertToHttpError(error) })
}
