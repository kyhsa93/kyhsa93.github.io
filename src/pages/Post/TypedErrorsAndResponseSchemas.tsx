...
if (!order) throw new Error(OrderErrorMessage['Order not found.'])`}</code></pre>
  ...
  ko: {
    ...
    lede: "throw new Error('Order not found.')는 완전히 멀쩡해 보인다 — 다른 어딘가에서 두 번째 사람이 문자열을 살짝 다르게 적기 전까지는. 그러면 그 순간부터 같은 실패가 어느 파일에서 던져졌는지에 따라 서로 다른 두 개의 코드로 나오게 된다.",
    body: (
      <>
        <p>에러 처리는 명확한 계층 분리를 갖고 있다: Domain과 Application 계층은 프레임워크 전용 HTTP 예외가 아니라 순수한 <code>Error</code>를 던지고, Interface 계층 — Controller — 만이 에러를 붙잡아 HTTP 상태 코드로 변환하는 유일한 지점이다. 이 분리 덕분에 Domain과 Application은 HTTP 의존성을 전혀 갖지 않게 되고, "이걸 상태 코드로 번역한다"는 지저분한 작업 하나가 정확히 한 곳으로 집중된다.</p>
        <pre><code>{`// domain/order.ts — inside the Aggregate
if (this._status === 'cancelled') throw new Error(OrderErrorMessage['This order has already been cancelled.'])
...
if (!order) throw new Error(OrderErrorMessage['Order not found.'])`}</code></pre>
  ...
  'An order must have at least one item.' = 'An order must have at least one item.',
        ...
        <p>Interface 계층의 변환은 런타임에 <code>error.message</code>를 이 enum 값들과 비교하는 방식으로 동작한다:</p>
        <pre><code>{`// The Interface layer's mapping
[OrderErrorMessage['Order not found.'], 404, OrderErrorCode.ORDER_NOT_FOUND]
//  ↑ the enum key (checked at compile time)      ↑ this value is compared against error.message at runtime`}</code></pre>
        <p>만약 키와 값이 서로 달라진다면 — 즉 누군가 enum을 거치지 않고 원본 문자열을 직접 적어버린다면 — 두 가지가 조용히 깨진다. 손으로 쓴 <code>throw new Error('Order not fund.')</code>의 오타는 단순 문자열 리터럴이기 때문에 컴파일 에러를 전혀 만들어내지 않는다. 그리고 별개로, Interface 계층이 <code>OrderErrorMessage['Order not found.']</code>와 비교할 때 더 이상 매치되지 않으므로, 그 에러는 원래 되어야 했던 404 대신 처리되지 않은 500으로 흘러가버린다. 키를 값과 동일하게 정의해두면 모든 throw 지점이 enum 키를 <em>거쳐</em> 가도록 강제된다 — <code>throw new Error(OrderErrorMessage['Order not found.'])</code> — 그러면 완전히 같은 오타라도 몇 주 뒤에 잘못된 상태 코드로 드러나는 대신 지금 당장 컴파일 에러로 잡힌다.</p>
  ...
  return this.orderQueryService.getOrder(param).catch((error) => {
    throw convertToHttpError(error.message, [
      ...
      [OrderErrorMessage['This order has already been cancelled.'], 400, OrderErrorCode.ORDER_ALREADY_CANCELLED]
        ...
        <p>이 매핑 테이블에 항목이 없는 에러는 500 Internal Server Error가 된다 — 이건 메꿔야 할 빈틈이 아니라 올바른 기본값이다. 매핑되지 않은 에러는 정말로 예상치 못한 실패이거나, Controller가 선언을 빠뜨린 도메인 에러 둘 중 하나를 의미한다 — 어느 쪽이든, 상태 코드를 추측해서 끼워맞추는 대신 불투명한 500으로 그대로 드러내는 쪽이 정직한 태도다.</p>
  ...
  "count": 42
        ...
        <p>Repository 쪽에서도 이 "범용 키를 쓰지 않는다"는 원칙이 한 계층 더 이어진다: 단일 레코드 조회는 별도 메서드로 만들지 않는다. 호출자는 같은 목록 조회 메서드에 <code>take: 1</code>을 넘기고 <code>.then()</code>으로 레코드를 꺼낸다:</p>
        <pre><code>{`const order = await this.orderRepository
  .findOrders({ orderId, take: 1, page: 0 })
  .then((r) => r.orders.pop())

if (!order) throw new Error(OrderErrorMessage['Order not found.'])`}</code></pre>
        <p>별도의 <code>findOne</code>을 유지하면 두 메서드 사이에 동적 필터 조건 로직이 중복될 뿐이다. 하나의 경로로 통합해두면 나중에 선택적 필터를 추가할 자리가 정확히 한 곳만 존재하게 된다.</p>
