...
export const meta: MetaFunction = () => createPostMeta('talking-across-bounded-contexts');
  ...
  en: {
    ...
    body: (
      <>
        ...
        <p>If the payment is later cancelled (<code>payment.cancelled.v1</code>), the Account BC subscribes the same way and runs a compensating credit that reverses the amount already deducted — not a transaction rollback, but a classic cross-BC compensating transaction: a new asynchronous event that offsets an earlier state change instead of undoing it in place. Refund approval (<code>refund.approved.v1</code>) reuses the exact same reaction. The real implementation lives at <code>implementations/go/examples/internal/application/event/payment_cancelled_event_handler.go</code>, reacting to the <code>PaymentCancelledV1</code> Integration Event defined in <code>internal/application/integration-event/</code>.</p>
  ...
  ko: {
    ...
    body: (
      <>
        ...
        <p>이건 기계적으로 검사할 수 있을 만큼 정확히 정의된 규칙이다: 모든 언어의 harness는 <code>no-cross-bc-repository-in-application</code> 규칙을 갖고 있으며, Application 계층 파일이 다른 BC의 Repository를 직접 import하면 이를 잡아낸다 — <em>같은</em> 도메인 안에서 Repository를 import하는 정상적인 패턴은 대상이 아니다.</p>
	...
	return h.orderRepository.SaveOrder(ctx, o)
        ...
        <div className="article-note"><strong>저장소 내 추가 자료</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/cross-domain-communication.md" target="_blank" rel="noreferrer">docs/architecture/cross-domain-communication.md</a> — 전체 판단 기준표와 Context Map 대응 관계 · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/implementations/go/examples/internal/application/event/payment_cancelled_event_handler.go" target="_blank" rel="noreferrer">payment_cancelled_event_handler.go</a> — 실제 보상 입금(compensating credit) 반응 로직
        </p></div>
      </>
    ),
  },
