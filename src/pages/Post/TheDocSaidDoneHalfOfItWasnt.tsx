...
export const meta: MetaFunction = () => createPostMeta('the-doc-said-done-half-of-it-wasnt');
...
const content = {
  en: {
    ...
    body: (
      <>
        ...
        <p>The next question was obvious once the first one paid off — what other conventions were documented but not enforced? Three more rounds followed, adding nine to eleven structural rules per language: domain-layer isolation, no cross-aggregate references within a Bounded Context, no direct env-var access outside config modules, aggregate-ID hex format, the exact four-field error-response shape, soft-delete filtering on every query, and more.</p>
  ...
  ko: {
    ...
    lede: 'kotlin 자신의 repository-pattern.md는 네이밍 정리가 끝났다고 말했다. 실제로는 쓰기 쪽 Repository만 끝났고, 짝을 이루는 읽기 쪽 Query 인터페이스는 전부 그대로였다 — 자동화된 검사가 없어서 살아남는 종류의 반쯤 끝난 수정이었다. 있어야 했던 검사를 만들자마자 다른 곳에서 진짜 위반 세 건이 더 나왔고, 네 라운드 뒤 아홉 개 더 많은 컨벤션에 같은 걸 반복했을 땐 언제 멈춰야 하는지까지 증명돼 있었다.',
    body: (
      <>
        ...
        <p>첫 번째가 성과를 내자 다음 질문은 자연스러웠다 — 문서화는 됐지만 강제되지 않는 컨벤션이 또 뭐가 있을까? 세 라운드가 더 이어졌고, 언어당 9~11개의 구조 규칙이 추가됐다: 도메인 레이어 격리, 같은 Bounded Context 내 크로스 애그리게이트 참조 금지, config 모듈 밖에서 env 변수 직접 접근 금지, 애그리게이트 ID의 16진수 포맷, 정확히 4필드인 에러 응답 형태, 모든 쿼리의 soft-delete 필터링 등.</p>
        ...
        <p>세 번째 라운드는 규칙 5개를 더하며 진짜 버그 2개를 더 찾았다 — fastapi의 <code>PaymentModel</code>과 <code>RefundModel</code>은 같은 코드베이스의 다른 모든 모델과 달리 <code>deleted_at</code> 컬럼 자체가 없었다. 이 라운드는 이전 라운드 기록이 "고쳐졌다"고 적어둔 것 하나를 반증하기도 했다: 이미 올바르게 배선됐다고 믿었던 java의 rate-limit filter가, 알고 보니 기록되지 않은 더 이전 커밋에서 실제로 이미 고쳐져 있었다 — 한 번 적힌 "알려진 갭"은 실시간 사실이 아니라 한 시점의 스냅샷일 뿐이니, 다시 인용하기 전에 현재 코드로 재확인해야 한다는 상기.</p>
