  ...
  ko: {
    ...
    body: (
      <>
        ...
        <p>메시지 브로커의 at-least-once 전달이란, 메시지가 Consumer에게 최소 한 번은 도달할 때까지 재시도한다는 뜻이다. 하지만 Consumer가 처리 결과를 저장한 직후, ack를 보내기 전에 죽어버리면 브로커는 같은 메시지를 다시 보낸다. 즉 전달이 보장된다고 해서 비즈니스 연산이 정확히 한 번만 실행된다는 보장은 되지 않는다.</p>
        <p>실무적으로는 at-least-once 전달을 그대로 받아들이고 Consumer를 멱등(idempotent)하게 만들어, effectively-once 처리를 목표로 하는 편이 현실적이다. 멱등성이란 같은 입력을 여러 번 처리해도 한 번 처리한 것과 동일한 결과가 관측된다는 뜻이다. 결제 승인, 포인트 적립, 이메일 발송처럼 외부에 영향을 미치는 연산일수록 이 경계가 중요하다.</p>
