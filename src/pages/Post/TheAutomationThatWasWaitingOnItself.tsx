...
export const meta: MetaFunction = () => createPostMeta('the-automation-that-was-waiting-on-itself');
...
const content = {
  en: {
    ...
    body: (
      <>
        ...
        <p>The approve call had never been doing anything useful in the first place — this repository has no required-review branch protection, so nothing was gated on an approval existing at all. It came out rather than getting worked around.</p>
        ...
        <p>The real test of whether any of this held up wasn't a green run watched live. It was noticing, later, in the middle of something unrelated, that one more routine dependency bump — PR #307 — had opened, passed its checks, and merged itself, with nobody watching it happen at all.</p>
        <div className="article-note"><strong>Further reading in the repo</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/.github/workflows/dependabot-auto-merge.yml" target="_blank" rel="noreferrer">.github/workflows/dependabot-auto-merge.yml</a> — the corrected workflow, self-check excluded, no approve step
        </p></div>
      </>
    ),
  },
  ko: {
    ...
    body: (
      <>
        <p>Dependabot PR 19개가 쌓여 있었고, 이걸 정리해야 할 auto-merge 워크플로는 서류상으로는 제 몫을 하는 것처럼 보였다 — 실제로 이력 중 몇몇 PR은 정말로 스스로 머지됐었다. 왜 애초에 백로그가 쌓였는지 파고들자, 가끔 실패하는 워크플로보다 더 나쁜 게 나왔다. 원래 의도한 이유로는 단 한 번도 성공한 적이 없던 워크플로였다.</p>
        <h2>자기 자신을 기다리던 잡</h2>
        ...
        <p>애초에 이 승인 호출은 처음부터 아무 쓸모도 없었다 — 이 저장소엔 필수 리뷰 브랜치 보호 자체가 없어서, 승인이 존재하는지에 걸려 있는 게 아무것도 없었다. 우회하지 않고 그냥 빠졌다.</p>
        <h2>502가 남기고 가는 것</h2>
        ...
        <p>이게 정말 버텨내는지에 대한 진짜 시험은 실시간으로 지켜본 초록 실행이 아니었다. 나중에, 완전히 다른 작업을 하던 도중에, 평범한 의존성 업데이트 하나 — PR #307 — 이 열리고, 체크를 통과하고, 아무도 지켜보지 않는 사이에 스스로 머지된 걸 알아챈 것이었다.</p>
        <div className="article-note"><strong>저장소 내 추가 자료</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/.github/workflows/dependabot-auto-merge.yml" target="_blank" rel="noreferrer">.github/workflows/dependabot-auto-merge.yml</a> — 자기 자신의 체크를 제외하고, 승인 스텝 없이 수정된 워크플로
        </p></div>
      </>
    ),
  },
