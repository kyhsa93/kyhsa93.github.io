...
export const UserContextStore = {
  ...
  ko: {
    ...
    body: (
      <>
        <p>인증(Authentication)은 계층형 아키텍처에서 있어야 할 자리가 명확하다: Interface 계층, 오직 그곳뿐이다. Domain 계층과 Application 계층은 인증 컨텍스트에 절대 의존하지 않는다 — Command나 Query는 그저 <code>userId</code> 같은 평범한 문자열처럼 필요한 값만 담고 있을 뿐, 그 ID가 어떻게 확립되었는지에 대해서는 아무것도 알지 못한다.</p>
        <pre><code>{`// forbidden — verifying the token directly in an Application Service
public async cancelOrder(token: string, command: CancelOrderCommand) {
  const user = await this.authService.verify(token)  // this is the Interface layer's job
  ...
...
public async cancelOrder(
  ...
  return this.commandService.cancelOrder({ ...body, userId: req.user.userId })
        ...
        <p>request 객체에서 바로 읽어오면 Handler와 그것이 호출하는 모든 것이, 단순히 전달받은 평범한 값이 아니라 "지금 HTTP 요청이 진행 중이다"라는 사실 자체에 결합된다. 이는 인증된 사용자 같은 필드에서 특히 더 큰 문제인데, 그 필드는 대개 <em>어디서든</em> 필요하기 때문이다: Handler 안에서, 때로는 Application 계층의 Service 안에서, 그리고 로깅이나 관측성(observability) 인터셉터 안에서도 다시 필요하다. 그 모든 호출 지점마다 <code>req</code>를 실로 꿰듯 전달하거나, 프레임워크 전역의 "현재 요청"을 끌어다 쓰는 것 둘 다, HTTP 메커니즘을 평범한 애플리케이션 호출로 변환한다는 Interface 계층 본연의 목적을 무너뜨린다.</p>
...
export const UserContextStore = {
  ...
  getRequesterId: (): string => {
    ...
    return user.userId
        ...
        <p>자연스럽게 이어지는 다음 질문은, 같은 아키텍처의 다른 네 언어 포트에도 동일한 함정이 있었는가다. 없었다 — 그리고 그 이유가 더 유용한 교훈이다. <code>req.user = payload</code>는 "이 요청에 대한 인증된 principal"이라는 개념이 프레임워크 자체에 없는 Express나 NestJS 같은 프레임워크 특유의, 잘 알려진 흔한 함정이다. 자체적인 요청 스코프 인증 추상화를 이미 갖고 있는 프레임워크들은 아무 변경 없이도 이미 올바른 상태였다: Go의 자체 <code>context.Context</code>와 <code>context.WithValue</code>, Spring Security의 <code>Authentication</code>을 명시적인 메서드 파라미터로 전달하는 방식(<code>HttpServletRequest</code>를 직접 건드리지 않음), 그리고 FastAPI의 <code>Depends(get_current_user)</code>. 모든 언어에 동일한 개입이 필요하다고 가정했다면 불필요한 수정 네 건을 추가로 진행했을 것이다. 각 언어의 실제 코드를 먼저 확인해보니 다섯 언어 중 실제로 변경이 필요했던 곳은 단 하나뿐이었다.</p>
