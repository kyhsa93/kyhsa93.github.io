...
export const meta: MetaFunction = () => createPostMeta('cqrs-in-practice');
...
func (h *GetOrdersHandler) Handle(ctx context.Context, q GetOrdersQuery) (*GetOrdersResult, error) {
	...
	return &GetOrdersResult{Orders: orders, Count: count}, nil
        ...
        <p>이건 그저 네이밍의 뉘앙스 차이처럼 보인다 — <code>order.Query</code> 대신 <code>order.Repository</code> — 그리고 바로 그 점이 아무도 눈치채지 못한 채 규칙을 어기기 쉽게 만든다. <code>Repository</code>는 <code>Query</code>를 임베드하고 있어서, 더 좁은 인터페이스도 함께 만족시킨다. 그 <code>*OrderRepository</code>는 이미 Command Handler에 연결되어 있고, 이미 테스트도 끝났고, 목록 화면이 필요로 하는 것을 정확히 반환하는 <code>FindOrders</code> 메서드까지 갖추고 있다 — 그런데 이 구현체를 그대로 Query Handler의 필드 타입으로 써도 타입 체크는 아무 문제 없이 통과해 버린다. 그 필드를 <code>order.Query</code> 대신 <code>order.Repository</code>로 선언해도 컴파일이 되고 리뷰도 통과하지만, CQRS가 애초에 닫으려 했던 바로 그 문을 조용히 다시 열어버린다: 이제 읽기 경로 바로 옆에 쓰기 능력(<code>SaveOrder</code>)이 놓이게 되고, 두 모델은 더 이상 실질적으로 분리되어 있지 않다.</p>
...
func (h *OrderHandler) GetOrders(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, r, result)
        ...
        <p>Domain Event는 여전히 여러 곳에 걸친(cross-cutting) 후속 작업을 위해 프로세스 내부 이벤트 버스를 쓰지 않는다 — 기반 아키텍처와 마찬가지로 Outbox → 메시지 큐 → EventConsumer 경로를 통해 전달된다. CQRS는 하나의 요청이 어떤 Handler로 라우팅되는지를 바꿀 뿐, 이미 일어난 사실이 이후에 어떻게 전파되는지는 바꾸지 않는다.</p>
        <h2>CQRS가 바꾸지 않는 것</h2>
        <p>기반 아키텍처와 Handler 기반 CQRS 모두 Domain 계층의 독립성, Aggregate의 캡슐화, Repository 패턴을 그대로 유지한다. CQRS는 그 토대 위에 얹히는 라우팅 및 읽기 모델에 관한 결정일 뿐, 그 토대를 대체하는 것이 아니다 — 그리고 바로 이 점이 Query Handler가 Repository를 가져다 쓰는 실수를 그렇게 쓰기 쉽고 그렇게 놓치기 쉽게 만드는 이유다: 그 아래의 모든 것은 여전히 컴파일되고, 여전히 유닛 테스트를 통과하며, 스스로 이미 조용히 벗어난 바로 그 아키텍처를 언뜻 보기에는 여전히 그대로 따르고 있는 것처럼 보인다.</p>
