...
export class RequestRefundCommandHandler {
  ...
  public async execute(command: RequestRefundCommand): Promise<Refund> {
    ...
    return refund
        ...
        <p><code>RefundEligibilityService</code>는 <code>new</code>로 직접 인스턴스화된다 — DI 컨테이너에 등록되는 일이 없으며, "상태를 갖지 않고 프레임워크 의존성도 없다"는 원칙을 그대로 지킨다. 이 클래스의 단위 테스트는 Application 계층을 아예 거치지 않는다. 클래스를 직접 <code>new</code>해서 판단 로직만 검증한다. 전체 코드는 <code>implementations/nestjs/examples/src/payment/domain/refund-eligibility-service.ts</code>에 있으며, 같은 위치에 <code>payment.ts</code>, <code>refund.ts</code>, 그리고 위의 command handler가 함께 있다.</p>
        <p>이 예시는 영구적인 회귀 방지 장치도 하나 얻었다: <code>payment/domain/</code> 내에서 <code>payment.ts</code>가 <code>Refund</code>를 타입으로 직접 import하지 않는지, 그리고 그 반대도 마찬가지인지를 검사하는 Harness 규칙이다 — 이는 두 Aggregate가 서로를 필드로 갖지 않고 오직 ID로만 참조한다는 것을 증명한다. <code>evaluate(payment: Payment, refund: Refund)</code>처럼 Domain Service가 둘을 함수 파라미터로 받는 정당한 패턴은 이 규칙의 검사 대상이 명시적으로 아니다.</p>
        <h2>Domain Service vs. Application Service vs. Technical Service</h2>
        ...
        <div className="article-note"><strong>저장소 내 추가 자료</strong><p>
          <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/docs/architecture/domain-service.md" target="_blank" rel="noreferrer">docs/architecture/domain-service.md</a> — Domain Service / Technical Service 패턴 전체와 위의 오용 사례 · <a href="https://github.com/kyhsa93/backend-service-playbook/blob/main/implementations/nestjs/examples/src/payment/domain/refund-eligibility-service.ts" target="_blank" rel="noreferrer">payment/domain/refund-eligibility-service.ts</a> — 실제 동작하는 코드
        </p></div>
      </>
    ),
  },
